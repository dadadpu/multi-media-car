...
myPushButton:: myPushButton(QString normal_path,QString press_path,int pixwidth,int pixheight)
{
   ...
   this->setFixedSize(pixwidth,pixheight);//设置大小
   this->setStyleSheet("QPushButton{border:0px;}");//这段代码的作用是将QPushButton的边框设置为0像素，即去掉按钮的边框。
   this->setIcon(pix);//设置图片

   this->setIconSize(QSize(pixwidth,pixheight));//设置图片大小
   this->setFocusPolicy(Qt::NoFocus);//取消焦点框，美观

   this->animation = new QPropertyAnimation(this,"geometry");//创建属性动画
